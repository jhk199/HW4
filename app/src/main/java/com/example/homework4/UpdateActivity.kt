...
// Literally the same layout as AddActivity with editable text
class UpdateActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val searchSpinner =
            ArrayAdapter(this, android.R.layout.simple_spinner_dropdown_item, emotionArray)
        searchSpinner.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
        selectSpinner.adapter = searchSpinner

        // getting intent
        val previousIntent = intent.getStringExtra("id") as String
        val id: Int = Integer.parseInt(previousIntent)
        ...
        editTextContents.setText(contents, TextView.BufferType.EDITABLE)
        editTextReflection.setText(reflection, TextView.BufferType.EDITABLE)
        for (i in 1..emotionArray.size - 1) {
            if(emotion.equals(emotionArray[i])) {
                selectSpinner.setSelection(i)
            }
        }
        ...
        buttonSaveDream.setOnClickListener{
            if(TextUtils.isEmpty(editTextName.text) || TextUtils.isEmpty(editTextDate.text)
                ...
                try {
                    ...
                    dreamViewModel.updateAll(id, editTextName.text.toString(), date.toString(),
